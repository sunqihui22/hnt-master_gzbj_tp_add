...
import com.shtoone.shtw.activity.PitchActivity;
import com.shtoone.shtw.adapter.OnItemClickListener;
import com.shtoone.shtw.adapter.PitchFragmentRecyclerViewAdapter;
 ...
 */
public class PitchFragment extends BaseLazyFragment {

    private static final String TAG = PitchFragment.class.getSimpleName();
    private Toolbar mToolbar;
    private PtrFrameLayout mPtrFrameLayout;
    private RecyclerView mRecyclerView;
    private PitchFragmentRecyclerViewAdapter mAdapter;
    private PitchFragmentRecyclerViewItemData itemsData;
    ...
    private void initData() {
        if (null != BaseApplication.parametersData) {
            ...
            mParametersData.fromTo = ConstantsUtils.PITCHFRAGMENT;
        }
        if (null != BaseApplication.mDepartmentData && !TextUtils.isEmpty(BaseApplication.mDepartmentData.departmentName)) {
            mDepartmentData = new DepartmentData(BaseApplication.mUserInfoData.getDepartId(), BaseApplication.mUserInfoData.getDepartName(), ConstantsUtils.PITCHFRAGMENT);
        }
        KLog.e(TAG,"沥青mParametersData=:"+mParametersData.toString());
        mGson = new Gson();
        setToolbarTitle();
    ...
    public String createRefreshULR() {
        ...
        return URL.getLQLingdaoData(userGroupID, startDateTime, endDateTime);
    }
    ...
    public void onRefreshSuccess(String response) {
        ...
        if (!TextUtils.isEmpty(response)) {
            ...
            if (jsonObject.optBoolean("success")) {
                itemsData = mGson.fromJson(response, PitchFragmentRecyclerViewItemData.class);
                if (null != itemsData) {
    ...
    public void updateSearch(ParametersData mParametersData) {
        ...
        if (mParametersData != null) {
            if (mParametersData.fromTo == ConstantsUtils.PITCHFRAGMENT) {
                fab.show();
    ...
    public void updateDepartment(DepartmentData mDepartmentData) {
        if (null != mDepartmentData && null != mParametersData && null != this.mDepartmentData) {
            if (mDepartmentData.fromto == ConstantsUtils.PITCHFRAGMENT) {
    ...
    private void setToolbarTitle() {
        if (null != mToolbar && null != mDepartmentData && !TextUtils.isEmpty(mDepartmentData.departmentName)) {
            ...
            sb.append(getString(R.string.asphalt)).trimToSize();
    ...
    private void setAdapter() {
        ...
        //设置动画
        SlideInLeftAnimationAdapter mSlideInLeftAnimationAdapter = new SlideInLeftAnimationAdapter(mAdapter = new PitchFragmentRecyclerViewAdapter(_mActivity, itemsData));
    ...
    private void jumpToPitchActivity(int position) {
        Intent intent = new Intent(_mActivity, PitchActivity.class);
        //这里不需要做健壮性判断，因为能走到这一步，itemsData必然有数据
        BaseApplication.mDepartmentData.departmentID = itemsData.getData().get(0).get(position).getDeptId();
        BaseApplication.mDepartmentData.departmentName = itemsData.getData().get(0).get(position).getDeptName();
        startActivity(intent);
    }
}
