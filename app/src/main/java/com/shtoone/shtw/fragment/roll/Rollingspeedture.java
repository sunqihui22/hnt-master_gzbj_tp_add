...
public class Rollingspeedture extends BaseFragment {

    private static final String TAG = Rollingspeedture.class.getSimpleName();
    ...
    private RollingSpeedRecylerView mAdapter;
    private FloatingActionButton    fab;
    private RecyclerView            mRecyclerView;
    private boolean isRegistered = false;

    private LineChart        mRollingTempareChart;
    private RollingspeedData data;

    private ParametersData      mParametersData;
    private  Gson                mGson;
    private  LinearLayoutManager mLinearLayoutManager;
    private List<RollingspeedData.DataEntity> mListData;
    ...
    public static Rollingspeedture newInstance() {
        return new Rollingspeedture();
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment_rollingspeed_statistic, container, false);
        initView(view);
        initData();
        return view;
    }
    ...
    private void initView(View view) {
        ...
        mNestedScrollView = (NestedScrollView) view.findViewById(R.id.nsv_rollingspeed_statistic_fragment);
        mPtrFrameLayout = (PtrFrameLayout) view.findViewById(R.id.ptr_rollingspeed_statistic_fragment);
        mPageStateLayout = (PageStateLayout) view.findViewById(R.id.psl_rollingspeed_statistic_fragment);
 //       mRollingTempareChart = (LineChart) view.findViewById(R.id.linechart1_rollingtempare_statistic_fragment);
    ...
    public String createRefreshULR() {
        ...
        return URL.getRollingspeedData(userGroupID,equipmentID, startDateTime,endDateTime,currentPage);
    }
    ...
    public void onRefreshSuccess(String response) {
        if (!TextUtils.isEmpty(response)) {
            ...
            if (jsonObject.optBoolean("success")) {
                data = mGson.fromJson(response, RollingspeedData.class);
                if (null != data) {
                    if (data.getSuccess() && data.getData().size() > 0) {
...
//        xAxis.setTextColor(Color.BLUE);
        ...
        //设置动画与适配器
        SlideInLeftAnimationAdapter mSlideInLeftAnimationAdapter = new SlideInLeftAnimationAdapter(mAdapter = new RollingSpeedRecylerView(_mActivity,mListData));
...
//        String[] x = data.getX();
        List<RollingspeedData.DataEntity> dataEntity = data.getData();
        for (int i = 0; i < dataEntity.size(); i++) {
            RollingspeedData.DataEntity dataEntity1 = dataEntity.get(i);
